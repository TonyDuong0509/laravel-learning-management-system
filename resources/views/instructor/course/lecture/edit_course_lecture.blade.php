...
<div class="page-content">
    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3"> 
        ...
        <div class="ms-auto">
            <div class="btn-group">
                <a href="{{ route('add.course.lecture', $c_lecture->course_id) }}">Back</a>
            </div>
        </div>
    ...
    <div class="card">
        <div class="card-body p-4">
            ...
            <form id="myForm" action="{{ route('update.course.lecture') }}" method="POST" class="row g-3" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="id" value="{{ $c_lecture->id }}">
                <div class="form-group col-md-6">
                    <label for="input1" class="form-label">Lecture Title</label>
                    <input type="text" name="lecture_title" class="form-control" id="input1" value="{{ $c_lecture->lecture_title }}" >
                </div>

                <div class="form-group col-md-6">
                    <label for="input1" class="form-label">Video Url </label>
                    <input type="text" name="url" class="form-control" id="input1" value="{{ $c_lecture->url }}" >
                </div>

                <div class="form-group col-md-12">
                    <label for="input1" class="form-label">Lecture Content </label>
                    <textarea name="content" class="form-control">{{ $c_lecture->content }}</textarea>
                </div>
